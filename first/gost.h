...
/* Секретный контейнер который сочитает в себе ключ и s-блоки */
typedef struct {
    u4 k[8];
    /* константы s-блоков */
...
/* Note: encrypt and decrypt expect full blocks--padding blocks is
         caller's responsibility. All bulk encryption is done in
                 ...
                 enough.
                                           */
/* Режимы шифрования */
void crypt( gost_ctx *ctx, const byte *in, byte *out );
void decrypt( gost_ctx *ctx, const byte *in, byte *out );
void gost_enc_ecb( gost_ctx *ctx, const byte *clear,    byte *cipher,                   int blocks );
void gost_dec_ecb( gost_ctx *ctx, const byte *cipher,   byte *clear,                    int blocks );
...
void gost_enc_ofb( gost_ctx *ctx, const byte *iv, const byte *clear,    byte *cipher,   int blocks );
void gost_dec_ofb( gost_ctx *ctx, const byte *iv, const byte *cipher,   byte *clear,    int blocks );
