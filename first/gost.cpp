...
/* Чать GOST 28147 алгоритма в отдельной функции */
...
/* GOST - главная функция дешифровки */
...
void gost_enc_ecb( gost_ctx *ctx, const byte *clear, byte *cipher, int blocks )
{
    int i;
    for ( i = 0; i < blocks; i++ ) {
        crypt( ctx, clear, cipher );
        clear += sb;
        cipher += sb;
    }
}

void gost_dec_ecb( gost_ctx *ctx, const byte *clear, byte *cipher, int blocks)
{
    int i;
    for ( i = 0; i < blocks; i++ ) {
        decrypt( ctx, clear, cipher );
        clear += sb;
        cipher += sb;
    }
}
...
void gost_dec_cbc( gost_ctx *ctx, const byte *iv, const byte *clear, byte *cipher, int blocks )
{
    byte cur_iv[sb];
    byte gamma[sb];
    int i, j;
...
void gost_dec_cfb( gost_ctx *ctx, const byte *iv, const byte *clear, byte *cipher, int blocks )
{
    byte cur_iv[sb];
    byte gamma[sb];
    int i, j;
...
void gost_dec_ofb( gost_ctx *ctx, const byte *iv, const byte *clear, byte *cipher, int blocks )
{
    byte cur_iv[sb];
    byte gamma[sb];
    int i, j;
...
void cryptopro_key_meshing(gost_ctx *ctx, unsigned char *iv)
        ...
        gost_dec_ecb(ctx,CryptoProKeyMeshingKey,newkey,4);
        /* set new key */
        gost_key(ctx,newkey);
