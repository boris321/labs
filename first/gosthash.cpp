 ...
 */
static int hash_step(gost_hash_ctx *ctx,gost_ctx *c,byte *H,const byte *M)
        {
...
int hash_block(gost_hash_ctx *ctx,const byte *block, size_t length)
        ...
        if (ctx->left)
                ...
                hash_step(ctx,ctx->cipher_ctx,ctx->H,ctx->remainder);
        ...
        while (curptr<=barrier)
                {
                hash_step(ctx,ctx->cipher_ctx,ctx->H,curptr);
...
int finish_hash(gost_hash_ctx *ctx,byte *hashval)
        ...
        if (ctx->left)
                ...
                hash_step(ctx,ctx->cipher_ctx,H,buf);
                add_blocks(32,S,buf);
                fin_len+=ctx->left;
                }
        memset(buf,0,32);
        ...
        hash_step(ctx,ctx->cipher_ctx,H,buf);
        hash_step(ctx,ctx->cipher_ctx,H,S);
        memcpy(hashval,H,32);
        return 1;
        }
